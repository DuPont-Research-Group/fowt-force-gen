...
class RoughTuneTest:
    ...
    def rough_tune_test_2(self):
        # Test OpenFAST input file creation for rough tuning
        mooring = moortune.Mooring(200, 'oc4')
        line_length = mooring.get_initial_line_length()
        mooring.update_tuning_inputs(line_length, test='rough')
        assert filecmp.cmp('rough_temp.fst', 'test_fast/compare_tune_rough.fst')
        assert filecmp.cmp('moordyn_temp.dat', 'test_fast/compare_tune_rough_moordyn.dat')
        assert filecmp.cmp('hydrodyn_rough_temp.dat', 'test_fast/compare_tune_rough_hydrodyn.dat')
        os.remove('moordyn_temp.dat')
        os.remove('hydrodyn_rough_temp.dat')
        os.remove('rough_temp.fst')

    def rough_tune_test_3(self):
        # Test true condition for rough tuning check
        mooring = moortune.Mooring(200, 'oc4')
        assert mooring.check_rough_tuning('test_fast/compare_tune_rough_nouplift.outb')

    def rough_tune_test_4(self):
        # Test false condition for rough tuning check
        mooring = moortune.Mooring(200, 'oc4')
        assert mooring.check_rough_tuning('test_fast/compare_tune_rough_uplift.outb')

class FineTuneTest:
    def fine_tune_test_1(self):
        ...
        compare_baseline_surge = np.genfromtxt('test_data/compare_baseline_surge.csv', delimiter=',')
        assert (baseline_time == compare_baseline_time).all()
        assert (baseline_surge == compare_baseline_surge).all()

    def fine_tune_test_2(self):
        ...
        assert filecmp.cmp('fine_temp.fst', 'test_fast/compare_tune_fine.fst')
        assert filecmp.cmp('moordyn_temp.dat', 'test_fast/compare_tune_fine_moordyn.dat')
        assert filecmp.cmp('hydrodyn_fine_temp.dat', 'test_fast/compare_tune_fine_hydrodyn.dat')
        os.remove('moordyn_temp.dat')
        os.remove('hydrodyn_fine_temp.dat')
        os.remove('fine_temp.fst')

    def fine_tune_test_3(self):
        # Test zero crossing identification for a test with the same number of crossings as baseline
        mooring = moortune.Mooring(200, 'oc4')
        baseline_time, baseline_surge = mooring.get_decay_data(mooring.baseline_outb_file)
        test_time, test_surge = mooring.get_decay_data('test_fast/compare_tune_fine_samecrossings.outb')
    ...
    def fine_tune_test_4(self):
        # Test zero crossing identification for a test with more crossings than baseline
        mooring = moortune.Mooring(200, 'oc4')
        baseline_time, baseline_surge = mooring.get_decay_data(mooring.baseline_outb_file)
        test_time, test_surge = mooring.get_decay_data('test_fast/compare_tune_fine_morecrossings.outb')
        error = mooring.compare_zero_crossings(baseline_time, test_time, baseline_surge, test_surge)
        print(error)
        compare_error = np.array([12.3, 37.2, 59.2, 81.8, 103.85])
        assert (np.around(error, 3) == compare_error).all()

    def fine_tune_test_5(self):
        # Test zero crossing identification for a test with less crossings than baseline
        mooring = moortune.Mooring(200, 'oc4')
        baseline_time, baseline_surge = mooring.get_decay_data(mooring.baseline_outb_file)
        test_time, test_surge = mooring.get_decay_data('test_fast/compare_tune_fine_fewercrossings.outb')
        error = mooring.compare_zero_crossings(baseline_time, test_time, baseline_surge, test_surge)
        compare_error = np.array([-110.85])
        assert (np.around(error, 3) == compare_error).all()

    def fine_tune_test_6(self):
    ...
    def fine_tune_test_7(self):
        ...
        tuned, line_adjust = mooring.check_fine_tuning(freq_error, prev_max_errors)
        compare_tuned = False
        compare_line_adjust = 10
        assert tuned == compare_tuned
        assert line_adjust == compare_line_adjust

    def fine_tune_test_8(self):
